  ...
  */
  ...
  #define HELP_stat "!staturls - check status of urls in memory"
  ...
  struct url_type {
    ...
    void print() {
      cout << "url_type(" << this->timestamp << " " << this->nick << " " << this->channel << " " << this->url << ")" << endl;
    }
  ...
  urlsniffer_staturls_cmd (NetServer *s)
  {
    ...
    int result = urlsniffer->urls_seen.size();
    SEND_TEXT(DEST, "%d URLs in memory!", result);
